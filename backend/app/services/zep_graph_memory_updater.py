...
Dynamically updates agent activity from simulations into the Zep graph.
"""
...
logger = get_logger('ops.zep_graph_memory_updater')
...
class AgentActivity:
    ...
    def to_episode_text(self) -> str:
        ...
        # Return directly in the `"agent_name: activity description"` format with no simulation prefix
    ...
    def _describe_like_post(self) -> str:
        """Like a post, including the original content and author information."""
    ...
    def _describe_dislike_post(self) -> str:
        """Dislike a post, including the original content and author information."""
        post_content = self.action_args.get("post_content", "")
        post_author = self.action_args.get("post_author_name", "")
    ...
    def _describe_repost(self) -> str:
        """Repost a post, including the original content and author information."""
    ...
    def _describe_quote_post(self) -> str:
        """Quote a post, including the original content, author, and quoted comment."""
        original_content = self.action_args.get("original_content", "")
        original_author = self.action_args.get("original_author_name", "")
    ...
    def _describe_follow(self) -> str:
        """Follow a user, including the followed user's name."""
    ...
    def _describe_create_comment(self) -> str:
        """Post a comment, including the comment content and the post being commented on."""
        content = self.action_args.get("content", "")
        post_content = self.action_args.get("post_content", "")
        post_author = self.action_args.get("post_author_name", "")
    ...
    def _describe_like_comment(self) -> str:
        """Like a comment, including comment content and author information."""
    ...
    def _describe_dislike_comment(self) -> str:
        """Dislike a comment, including comment content and author information."""
        comment_content = self.action_args.get("comment_content", "")
        comment_author = self.action_args.get("comment_author_name", "")
...
class ZepGraphMemoryUpdater:
    ...
    Watches the simulation `actions` log and updates new agent activity into the Zep graph in real time.
    Activities are grouped by platform and sent to Zep in batches whenever `BATCH_SIZE` items accumulate.

    All meaningful actions are sent to Zep, and `action_args` includes the full context:
    - Original content of liked/disliked posts
    - Original content of reposted/quoted posts
    - Usernames that were followed or muted
    ...
    # Send interval in seconds, to avoid sending requests too quickly
    SEND_INTERVAL = 0.5
    ...
    def __init__(self, graph_id: str, api_key: Optional[str] = None):
        ...
        Args:
            ...
            api_key: Optional Zep API key; defaults to the configured value
        """
        ...
        # Platform-specific activity buffers; each platform sends once it reaches `BATCH_SIZE`
        ...
        self._skipped_count = 0     # Filtered-out activities, such as `DO_NOTHING`
    ...
    def add_activity(self, activity: AgentActivity):
        ...
        - `QUOTE_POST` (quoted posts)
        - `SEARCH_POSTS` (post searches)
        - `SEARCH_USER` (user searches)
        - `LIKE_POST` / `DISLIKE_POST` (post reactions)
        - `REPOST` (reposts)
        - `FOLLOW` (follows)
        - `MUTE` (mutes)
        - `LIKE_COMMENT` / `DISLIKE_COMMENT` (comment reactions)

        `action_args` includes the full context, such as original post content and usernames.
    ...
    def _worker_loop(self):
        """Background worker loop that sends activities to Zep in platform-specific batches."""
        while self._running or not self._activity_queue.empty():
            try:
                # Try to get an activity from the queue, with a 1-second timeout
                try:
                    activity = self._activity_queue.get(timeout=1)
                    
                    # Add the activity to the matching platform buffer
                    ...
                    with self._buffer_lock:
                        ...
                        if len(self._platform_buffers[platform]) >= self.BATCH_SIZE:
                            ...
                            # Pause between sends to avoid sending requests too quickly
    ...
    def _send_batch_activities(self, activities: List[AgentActivity], platform: str):
        """
        Send activities to the Zep graph in a batch, merging them into one text block.
    ...
    def _flush_remaining(self):
        """Send any remaining activities from the queue and buffers."""
        # First move any remaining queued activities into the platform buffers
        ...
        # Then send the remaining activities in each platform buffer, even if the count is below `BATCH_SIZE`
        with self._buffer_lock:
            for platform, buffer in self._platform_buffers.items():
                if buffer:
                    display_name = self._get_platform_display_name(platform)
                    logger.info(f"Sending the remaining {len(buffer)} activities for {display_name}")
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "skipped_count": self._skipped_count,        # Number of filtered-out activities (`DO_NOTHING`)
...
class ZepGraphMemoryManager:
    """
    Manage multiple Zep graph memory updaters for different simulations.
    ...
    def create_updater(cls, simulation_id: str, graph_id: str) -> ZepGraphMemoryUpdater:
        ...
        """
        with cls._lock:
            # Stop the old updater first if one already exists

...
logger = get_logger('ops.zep_entity_reader')
...
class ZepEntityReader:
    ...
    def get_node_edges(self, node_uuid: str) -> List[Dict[str, Any]]:
        """
        Get all related edges for a specific node, with retry logic.
    ...
    def filter_defined_entities(
        ...
        - If a node's labels include tags other than `"Entity"` and `"Node"`, it matches a predefined type and is kept
        
        Args:
            graph_id: Graph ID
            defined_entity_types: Optional list of predefined entity types; if provided, only those types are kept
        ...
        # Fetch all edges for later relation lookup
        ...
        for node in all_nodes:
            ...
            # Filtering rule: labels must include a tag other than "Entity" and "Node"
            custom_labels = [l for l in labels if l not in ["Entity", "Node"]]
            
            if not custom_labels:
                # Only default labels are present; skip the node
                continue
            
            # If predefined types were specified, check whether the node matches
            ...
            if enrich_with_edges:
                ...
                # Fetch basic information for connected nodes
                related_nodes = []
    ...
    def get_entity_with_context(
        ...
        """
        Get a single entity and its full context, including edges and related nodes, with retry logic.
        ...
        try:
            ...
            # Fetch all nodes for relation lookup
            all_nodes = self.get_all_nodes(graph_id)
            node_map = {n["uuid"]: n for n in all_nodes}
            
            # Build related edge and node data
            related_edges = []
            related_node_uuids = set()
            ...
            # Fetch related node information
            related_nodes = []
            for related_uuid in related_node_uuids:
                if related_uuid in node_map:
                    related_node = node_map[related_uuid]

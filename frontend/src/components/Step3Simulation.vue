    ...
    <div class="control-bar">
      ...
      <div class="action-controls">
        ...
        >
          ...
          {{ isGeneratingReport ? 'Starting up...' : 'Start Generating Result Report' }} 
          ...
          >
            ...
            <div class="timeline-card">
              ...
              <div class="card-body">
                ...
                <!-- General fallback: Unknown type or has content but not handled by the above -->
...
const actionIds = ref(new Set()) // A set of unique action IDs for deduplication
...
// Count of actions on each platform
...
// Simulation elapsed time on Twitter platform
const twitterElapsedTime = computed(() => {
  return formatElapsedTime(runStatus.value.twitter_current_round || 0)
})

// Simulation elapsed time on Reddit platform
...
const doStartSimulation = async () => {
  ...
  // Reset all states to ensure no carryover from previous simulations
  resetAllState()
  
  isStarting.value = true
  startError.value = null
  addLog('Starting parallel simulation across both platforms...')
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`✗ Simulation start failed: ${res.error || 'Unknown error'}`)
      emit('update-status', 'error')
    }
  } catch (err) {
    startError.value = err.message
    addLog(`✗ Simulation start error: ${err.message}`)
    emit('update-status', 'error')
  } finally {
    isStarting.value = false
  }
...
// Track the last round for each platform to detect changes and log them
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Detect changes in rounds for each platform and log them
      if (data.twitter_current_round > prevTwitterRound.value) {
        addLog(`[Plaza] R${data.twitter_current_round}/${data.total_rounds || '-'} | T:${twitterElapsedTime.value} | A:${data.twitter_actions_count || 0}`)
        prevTwitterRound.value = data.twitter_current_round
      }
      
      if (data.reddit_current_round > prevRedditRound.value) {
        addLog(`[Community] R${data.reddit_current_round}/${data.total_rounds || '-'} | T:${redditElapsedTime.value} | A:${data.reddit_actions_count || 0}`)
        prevRedditRound.value = data.reddit_current_round
      }
      
      // Check if the simulation has completed (determined by runner_status or platform completion status)
      const isCompleted = data.runner_status === 'completed' || data.runner_status === 'stopped'
      
      // Additional check: If backend hasn't updated runner_status yet but platforms have reported completion
      // Determine this through twitter_completed and reddit_completed statuses
    ...
    console.warn('Failed to get running status:', err)
  }
}

// Check if all enabled platforms have completed
const checkPlatformsCompleted = (data) => {
  // If no platform data is available, return false
  ...
  // Determine if a platform is enabled by actions_count (if count > 0 or running was ever true)
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Do not automatically scroll, let the user freely view the timeline
...
const handleNextStep = async () => {
  ...
  if (isGeneratingReport.value) {
    addLog('A request to generate the report has been sent, please wait...')
    return
  }
  ...
  try {
    ...
    addLog(`✗ An error occurred while starting report generation: ${err.message}`)
    isGeneratingReport.value = false
  }
}

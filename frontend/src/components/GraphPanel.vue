  ...
  <div class="graph-panel">
    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <!-- Building/simulation in progress prompt -->
        <div v-if="currentPhase === 1 || isSimulating" class="graph-building-hint">
          ...
          {{ isSimulating ? 'GraphRAG Long-Short Term Memory Real-Time Updating' : 'Updating in real time...' }}
        </div>
        ...
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">Some Content is Still Being Processed, Manual Refresh of the Graph is Recommended Later</span>
          <button class="hint-close-btn" @click="dismissFinishedHint" title="Close Notification">
    ...
    <!-- Toggle switch for displaying edge labels -->
...
const showEdgeLabels = ref(true) // Default display of edge labels
const expandedSelfLoops = ref(new Set()) // Expanded self-loop items
const showSimulationFinishedHint = ref(false) // Post-simulation prompt
const wasSimulating = ref(false) // Track if simulation was running before

// Close post-simulation prompt
const dismissFinishedHint = () => {
  showSimulationFinishedHint.value = false
}

// Listen to isSimulating changes, detect end of simulation
watch(() => props.isSimulating, (newValue, oldValue) => {
  if (wasSimulating.value && !newValue) {
    // Change from simulated state to non-simulated state, display 'end' prompt
    showSimulationFinishedHint.value = true
  }
  wasSimulating.value = newValue
}, { immediate: true })

// Toggle the expand/collapse state of self-loop items
...
// Calculate entity types for the legend
...
const renderGraph = () => {
  ...
  // Process edge data, calculate the number and index of edges between the same pair of nodes
  const edgePairCount = {}
  const selfLoopEdges = {} // Self-loops grouped by node
  const tempEdges = edgesData
    .filter(e => nodeIds.has(e.source_node_uuid) && nodeIds.has(e.target_node_uuid))
  
  // Count the number of edges between each pair of nodes, collect self-loops
  tempEdges.forEach(e => {
    if (e.source_node_uuid === e.target_node_uuid) {
      // Self-loops - gather into an array
  ...
  // Record which edge is currently being processed for each pair of nodes
  const edgePairIndex = {}
  ...
  tempEdges.forEach(e => {
    ...
    if (isSelfLoop) {
      // Self-loops - add only one merged loop per node
      ...
      edges.push({
        ...
        rawData: {
          ...
          selfLoopEdges: allSelfLoops // Store detailed information for all self-loops
        }
      })
      return
    }
    ...
    // Determine if the direction of the edge matches the normalized direction (source UUID < target UUID)
    const isReversed = e.source_node_uuid > e.target_node_uuid
    
    // Calculate curvature: spread out when multiple edges, straight line when single edge
    let curvature = 0
    if (totalCount > 1) {
      // Evenly distribute curvature to ensure clear distinction
      // Increase curvature range proportionally with the number of edges, larger number means wider range
      const curvatureRange = Math.min(1.2, 0.6 + totalCount * 0.15)
      curvature = ((currentIndex / (totalCount - 1)) - 0.5) * curvatureRange * 2
      
      // If the edge direction is opposite to the normalized direction, flip the curvature.
      // This ensures that all edges are distributed in the same reference frame and do not overlap due to different directions.
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  const simulation = d3.forceSimulation(nodes)
    .force('link', d3.forceLink(edges).id(d => d.id).distance(d => {
      // Adjust the distance between nodes based on the number of edges between a pair of nodes.
      // Base distance is 150, increase by 40 for each additional edge.
    ...
    // Add a centripetal force to gather independent node clusters towards the center area.
  ...
  // Links - Use path support for curves.
  ...
  const getLinkPath = (d) => {
    ...
    if (d.isSelfLoop) {
      ...
      const loopRadius = 30
      // Start from the right side of the node, make a full circle back.
      const x1 = sx + 8  // Offset at the start point
      const y1 = sy - 4
      const x2 = sx + 8  // Offset at the end point
      const y2 = sy + 4
      // Draw the self-loop using an arc (sweep-flag=1 for clockwise)
    ...
    // Calculate curve control points - dynamically adjust based on the number of edges and distance.
    const dx = tx - sx, dy = ty - sy
    const dist = Math.sqrt(dx * dx + dy * dy)
    // Offset perpendicular to the connection direction, calculated as a proportion of the distance to ensure the curve is clearly visible.
    // The greater the number of edges, the higher the percentage of offset relative to the distance.
  ...
  // Calculate the midpoint of the curve (for label positioning)
  const getLinkMidpoint = (d) => {
    ...
    // Quadratic Bézier curve formula: B(t) = (1-t)²P0 + 2(1-t)tP1 + t²P2, where t=0.5
  ...
  // Link label background (a white background makes text clearer)
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      .on('start', (event, d) => {
        // Only record positions without restarting simulation (distinguish between clicks and drags)
        d.fx = d.x
        d.fy = d.y
      ...
      .on('drag', (event, d) => {
        // Detect if actual dragging has started (movement exceeds a threshold)
        const dx = event.x - d._dragStartX
        const dy = event.y - d._dragStartY
        const distance = Math.sqrt(dx * dx + dy * dy)
        
        if (!d._isDragging && distance > 3) {
          // Restart the simulation only when true dragging is first detected
          d._isDragging = true
      ...
      .on('end', (event, d) => {
        // Allow the simulation to gradually stop only after true dragging occurs
        if (d._isDragging) {
  ...
  simulation.on('tick', () => {
    ...
    linkLabels.each(function(d) {
      ...
      d3.select(this)
        .attr('x', mid.x)
        .attr('y', mid.y)
        .attr('transform', '') // Remove rotation, keep it horizontal
    })
  ...
  // Click on the blank area to close the details panel
  svg.on('click', () => {
    selectedItem.value = null
    node.attr('stroke', '#fff').attr('stroke-width', 2.5)
    linkGroup.selectAll('path').attr('stroke', '#C0C0C0').attr('stroke-width', 1.5)
...
// Listen for the toggle switch of sidebar display

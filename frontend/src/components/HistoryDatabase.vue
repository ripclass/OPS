<template>
  ...
  >
    <!-- Background decoration: Technical grid lines (only displayed when there are items) -->
    ...
    <div v-if="projects.length > 0" class="cards-container" :class="{ expanded: isExpanded }" :style="containerStyle">
      ...
      >
        <!-- Card header: simulation_id and functionality availability status -->
        ...
        <div class="card-files-wrapper">
          ...
          <div class="files-list" v-if="project.files && project.files.length > 0">
            ...
            <!-- If more files are available, display a hint -->
        ...
        <!-- Card title (use the first 20 characters of the simulated requirement as the title) -->
        <h3 class="card-title">{{ getSimulationTitle(project.simulation_requirement) }}</h3>

        <!-- Card description (fully display the simulated requirement) -->
...
const selectedProject = ref(null)  // Currently selected item (for popups)
let observer = null
let isAnimating = false  // Animation lock, prevent flickering
let expandDebounceTimer = null  // Debounce timer
let pendingState = null  // Record target status to be executed

// Card layout configuration - adjust to a wider ratio
...
const containerStyle = computed(() => {
  ...
  // Expanded state: dynamically calculate height based on the number of cards
  ...
  // Calculate the actual required height: row count * card height + (row count-1) * spacing + minimal bottom margin
...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    // Calculate the number of cards in the current row to ensure centered alignment per row
    ...
    // Expand downwards, increase spacing from the title
    ...
    const x = offset * 35
    // Adjust starting position to be close to the title but maintain an appropriate margin
...
// Get style class based on the round progress
...
// Format date (only show the date part)
...
// Format simulation_id display (truncate to the first 6 digits)
...
// Format the round display (current round / total rounds)
...
// Truncate filename (keep extension only)
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Update target state for execution (record the latest target state regardless of whether it's in an animation)
        pendingState = shouldExpand
        
        // Clear previous debounce timer (new scroll intent will override old one)
        if (expandDebounceTimer) {
          clearTimeout(expandDebounceTimer)
          expandDebounceTimer = null
        }
        
        // Only record state if currently in an animation, handle after animation ends
        if (isAnimating) return
        
        // Do not process if target state is the same as current state
        if (shouldExpand === isExpanded.value) {
          pendingState = null
          return
        }
        
        // Use debounce to delay state switching to prevent rapid flickering
        // Shorter delay for expanding (50ms), longer delay for collapsing (200ms) for increased stability
        const delay = shouldExpand ? 50 : 200
        
        expandDebounceTimer = setTimeout(() => {
          // Check if currently in an animation
          if (isAnimating) return
          
          // Check if execution state still needs to be executed (may have been overridden by subsequent scroll)
          ...
          // remove lock after animation completes and check for any pending state changes
          setTimeout(() => {
            isAnimating = false
            
            // check for new states to execute after the animation ends
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // delay execution by a short period to avoid rapid switching
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState
    ...
    },
    {
      ...
      // adjust rootMargin to contract the viewport bottom up, requiring more scrolling before expansion
...
// listen for route changes and reload data when returning to the home page
...
onMounted(async () => {
  // ensure data loading occurs only after DOM rendering is complete
  await nextTick()
  await loadHistory()
  
  // wait for DOM rendering before initializing the observer
  setTimeout(() => {
    initObserver()
  }, 100)
})

// reload data when using keep-alive and component activation
onActivated(() => {
  loadHistory()
})
...
/* Simplify display when there are no items */
...
/* Create a fixed-spaced square grid using CSS background images */
.grid-pattern {
  position: absolute;
  top: 0;
  left: 0;
  ...
  /* Position starting from the top-left corner, expand only at the bottom when the height changes without affecting existing grid positions */
...
.cards-container {
  ...
  /* Min-height dynamically calculated by JS based on the number of cards */
}
...
/* ====== Replay history detail popup style ====== */
.modal-overlay {
  position: fixed;
  top: 0;
  left: 0;

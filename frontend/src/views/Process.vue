<template>
  <div class="process-page">
    <!-- Top navigation bar -->
    <nav class="navbar">
      <div class="nav-brand" @click="goHome">Murmur</div>
    ...
    <div class="main-content">
      ...
      <div class="left-panel" :class="{ 'full-screen': isFullScreen }">
        ...
        <div class="graph-container" ref="graphContainer">
          <!-- Spectrum visualization (display if there is data) -->
          <div v-if="graphData" class="graph-view">
            <svg ref="graphSvg" class="graph-svg"></svg>
            <!-- Building hint -->
            <div v-if="currentPhase === 1" class="graph-building-hint">
              <span class="building-dot"></span>
              Real-time updating in progress...
            </div>
        ...
        <div class="process-content">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                ...
                <div class="detail-content">
                  After uploading the document, LLM analyzes its content to automatically generate an ontology structure suitable for public opinion simulation (entity types + relationship types)
                </div>
              </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            <div class="phase-header">
              ...
              <div class="phase-info">
                <div class="phase-title">Knowledge Graph Construction</div>
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">Interface Description</div>
                <div class="detail-content">
                  Based on the generated ontology, chunk the documents and call the Zep API to construct the knowledge graph, extracting entities and relationships
                </div>
              </div>
              
              <!-- Waiting for Ontology Completion -->
              <div class="detail-section waiting-state" v-if="currentPhase < 1">
                <div class="waiting-hint">Waiting for the ontology generation to complete...</div>
              </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 2, 'completed': currentPhase > 2 }">
            <div class="phase-header">
              ...
              <div class="phase-info">
                ...
                <div class="phase-api">Preparing to Proceed to Next Step</div>
              </div>
...
const statusText = computed(() => {
  ...
  if (currentPhase.value === 1) return 'Spectrogram building in progress'
...
const goToNextStep = () => {
  // TODO: Enter environment setup steps
...
// initialize - handle new project creation or existing project loading
const initProject = async () => {
  const paramProjectId = route.params.projectId
  
  if (paramProjectId === 'new') {
    // Create a new project: retrieve the data to be uploaded from store
    ...
    // Handle creating a new project - call the ontology/generate API
const handleNewProject = async () => {
  ...
  try {
    ...
    if (response.success) {
      ...
      // Update URL (do not refresh the page)
      router.replace({
        name: 'Process',
...
const loadProject = async () => {
  try {
    ...
    if (response.success) {
      ...
      // Continue polling for the building tasks
...
const startBuildGraph = async () => {
  try {
    ...
    if (response.success) {
      buildProgress.value.message = 'The graph spectrum construction task has started...'
      
      // Save the task_id for polling
      const taskId = response.data.task_id
      
      // Start polling of graph data (independent of task status polling)
      startGraphPolling()
      
      // Start polling of task status
      startPollingTask(taskId)
    } else {
      error.value = response.error || 'Failed to start graph spectrum construction'
      buildProgress.value = null
    }
  } catch (err) {
    console.error('Build graph error:', err)
    error.value = 'Failed to start graph spectrum construction: ' + (err.message || 'Unknown error')
    buildProgress.value = null
  }
}

// Timer for graph data polling
let graphPollTimer = null

// Start polling of graph data
const startGraphPolling = () => {
  // Immediately get once
  fetchGraphData()
  
  // Automatically get the chart data every 10 seconds
...
const fetchGraphData = async () => {
  try {
    // First, get the project information to obtain graph_id
...
const startPollingTask = (taskId) => {
  // Execute a query immediately once
  ...
  try {
    ...
    if (response.success) {
      ...
      if (task.status === 'completed') {
        ...
        buildProgress.value = {
          progress: 100,
          message: 'Build completed, loading graph now...'
        }
        ...
        if (projectResponse.success) {
          ...
          // Final step: load complete graph data
          if (projectResponse.data.graph_id) {
            console.log('📊 Loaded full graph:', projectResponse.data.graph_id)
...
const renderGraph = () => {
  ...
  // Create a name mapping for lookup
  const nodeMap = {}
  nodesData.forEach(n => {
    nodeMap[n.uuid] = n
  })
  ...
  const nodes = nodesData.map(n => ({
    ...
    rawData: n // Save the original data
  }))
  
  // Create a set of node IDs for filtering valid edges
  ...
  // Draw edges (including transparent thick lines that are clickable)
  ...
  // A transparent thick line for clicking
  ...
  // Clicking on empty space to close the details panel
  svg.on('click', () => {
    closeDetailPanel()
  })
  
  simulation.on('tick', () => {
    // Update the positions of all edges (including visible lines and transparent click areas)
    ...
    // Update the position of edge labels
    linkLabel
...
// Listen for changes in graph data
watch(graphData, () => {
  if (graphData.value) {
...
['/* Edge Detail Relationships Display */']
.edge-relation {
  display: flex;
  align-items: center;
  flex-wrap: wrap;
...
['/* Progress of the main body generation */']
.ontology-progress {
  display: flex;
  align-items: center;
  gap: 12px;
